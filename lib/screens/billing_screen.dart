...
class _BillingScreenState extends State<BillingScreen> {
  ...
  void initState() {
    ...
    // Check if fields are already filled (e.g., from previous session)
  ...
  Future<void> _handlePayment() async {
    ...
    try {
      ...
      // NOTE: Razorpay plugin only works on Android/iOS, not on web/desktop.
      // On unsupported platforms we show a clear message instead of hanging.
      if (!Theme.of(context).platform.toString().toLowerCase().contains('android') &&
          !Theme.of(context).platform.toString().toLowerCase().contains('ios')) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
